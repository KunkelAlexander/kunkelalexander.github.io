...
- the columns represent the different board configurations (the states),
- the rows represent the different moves (the actios),
- each entry represents a numerical score: the Q-value that tells you how good an action is in a given state
...
- White cells indicate invalid moves (occupied positions).
- I excluded all states with zero moves in this visualisation.
...
- States are ordered by number of valid moves left.
- Many entries are orange: this corresponds to the initial Q-value of $$0.6$$, chosen to balance exploration and conservatism.
- Early states with few or no visits such as the ones on the left retain their initial values.
- The agent strongly favors the center unless it's already taken—then it prefers corners.
- As more opponent moves are added, the agent learns to avoid traps and block wins.
- Final states may include Q-values of $$-1$$ (unavoidable loss), $$0$$ (draw), or $$1$$ (win).
...
- More zero or negative Q-values.
- High pessimism: the minimax agent assumes the opponent plays optimally.
- Q-values reflect draws in most 8-move states.
- More losing positions are marked compared to the optimistic Q-agent trained on random play.
...
- Maximum Q-values are close to $$0$$ in both cases and match
- Q-agent is more optimistic about a number of losing positions after
- Q-agent reliably has learned to reliably play draws against the deterministic minimax agent
